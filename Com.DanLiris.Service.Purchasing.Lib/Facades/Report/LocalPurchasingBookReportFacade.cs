...
namespace Com.DanLiris.Service.Purchasing.Lib.Facades.Report
{
    public class LocalPurchasingBookReportFacade : ILocalPurchasingBookReportFacade
    {
        ...
        //        query = query.Where(urn => unitFilterIds.Contains(int.Parse(urn.UnitId)));
        ...
        //        query = query.Where(urn => categoryFilterIds.Contains(int.Parse(urn.CategoryId)));

        //    var queryResult = query.OrderByDescending(urn => urn.ReceiptDate).ToList();
        //    //var currencyCodes = queryResult.Select(item => item.CurrencyCode).ToList();
        //    var currencyTuples = queryResult.Select(item => new Tuple<string, DateTimeOffset>(item.CurrencyCode, item.ReceiptDate));
        //    var currencies = await _currencyProvider.GetCurrencyByCurrencyCodeDateList(currencyTuples);
        ...
        //            Remark = item.Remark,
        //            IncomeTax = incomeTax,
        //            IncomeTaxCurrency = incomeTax * (decimal)currencyRate,
        //            IncomeTaxBy = item.IncomeTaxBy
        //        };
        ...
        //                    Category = report.Key.AccountingCategoryName,
        //                    SubTotal = report.Sum(sum => sum.Total),
        //                    SupplierIncomeTax = report.Where(sum => sum.IncomeTaxBy == "Supplier").Sum(sum => sum.IncomeTax),
        //                    SupplierIncomeTaxCurrency = report.Where(sum => sum.IncomeTaxBy == "Supplier").Sum(sum => sum.IncomeTaxCurrency),
        //                    AccountingLayoutIndex = report.Select(item => item.AccountingLayoutIndex).FirstOrDefault()
        //                }).OrderBy(order => order.AccountingLayoutIndex).ToList();
        //    reportResult.CurrencySummaries = reportResult.Reports
        //        .GroupBy(report => new { report.CurrencyCode })
        ...
        //            if (isValas)
        //            {
        //                reportDataTable.Rows.Add(report.ReceiptDate.ToString("dd/MM/yyyy"), report.SupplierName, report.ProductName, report.IPONo, report.DONo, report.URNNo, report.InvoiceNo, report.VATNo, report.UPONo, report.AccountingCategoryCode + " - " + report.AccountingCategoryName, report.AccountingUnitName, report.Quantity, report.Uom, report.CurrencyCode, report.CurrencyRate, report.DPP, report.VAT, report.Total);
        //            }
        //            else
        //            {
        //                reportDataTable.Rows.Add(report.ReceiptDate.ToString("dd/MM/yyyy"), report.SupplierName, report.ProductName, report.IPONo, report.DONo, report.URNNo, report.InvoiceNo, report.VATNo, report.UPONo, report.AccountingCategoryCode + " - " + report.AccountingCategoryName, report.AccountingUnitName, report.Quantity, report.Uom, report.CurrencyCode, report.DPP, report.VAT, report.Total);
        //            }
        //        }
        //        foreach (var categorySummary in result.CategorySummaries)
        //            categoryDataTable.Rows.Add(categorySummary.Category, categorySummary.SubTotal);
