    ...
    public class GarmentUnitReceiptNoteFacade : IGarmentUnitReceiptNoteFacade
    {
            ...
            using (var transaction = dbContext.Database.BeginTransaction())
            {
                try
                {
                    ...
                    foreach (var garmentUnitReceiptNoteItem in garmentUnitReceiptNote.Items)
                    {
                        ...
                        var garmentDeliveryOrderDetail = dbSetGarmentExternalPurchaseOrderItems.First(d => d.Id == garmentUnitReceiptNoteItem.DODetailId);
        ...
        public async Task<int> Delete(int id)
        {
            ...
            using (var transaction = dbContext.Database.BeginTransaction())
            {
                try
                {
                    var garmentUnitReceiptNote = dbSet.Single(m => m.Id == id);
